...
import { ChatRoom, ChatMessage } from "@ng-console/shared/models";
...
describe('ChatService', () => {
  ...
  beforeEach(() => {
    mockIo.mockClear();
    Object.values(mockSocket).forEach(mockFn => (mockFn as Mock).mockClear());
  ...
  describe('WebSocket event handlers', () => {
    ...
    let readReceiptUpdatedHandler: (data: {
      roomId: string;
      userId: string;
      lastReadAt: string;
    }) => void;
    ...
    beforeEach(() => {
      ...
      readReceiptUpdatedHandler = mockSocket.on.mock.calls.find(
        (c) => c[0] === 'readReceiptUpdated',
      )?.[1];
    ...
    it('should handle newMessage event for inactive room', () => {
      (service.rooms as WritableSignal<ChatRoom[]>).set([
        { id: '2', name: 'Room 2', hasUnread: false, createdAt: '2023', updatedAt: '2023' },
      ]);
      (service.activeRoomId as WritableSignal<string>).set('1');
  ...
  describe('HTTP calls', () => {
    ...
    it('should fetch rooms', () => {
      const mockRooms: ChatRoom[] = [
        { id: '1', name: 'Room 1', hasUnread: false, createdAt: timestamp, updatedAt: timestamp },
      ];
      service.fetchRooms();

      const req = httpMock.expectOne(
        (req) =>
          req.url.includes(`${environment.apiUrl}/api/chats/rooms?_t=`) && req.method === 'GET',
      );
      req.flush(mockRooms);
  ...
  describe('selectRoom and fetch messages', () => {
    it('should set active room, clear old messages, fetch new messages, and mark as read', () => {
      const mockMessages: ChatMessage[] = [
        { id: '100', roomId: '1', senderId: 'u1', content: 'hello', createdAt: '2023' },
      ];
      ...
      const req = httpMock.expectOne(
        (req) =>
          req.url.includes(`${environment.apiUrl}/api/chats/rooms/1/messages?_t=`) &&
          req.method === 'GET',
      );
